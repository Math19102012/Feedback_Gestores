...
st.markdown(f"<h5 style='color:white;'>📊 Total de Relatórios Filtrados: {len(df_validos)}</h5>", unsafe_allow_html=True)
...
df_total_long = pd.melt(df_filtrado_geral, id_vars=['RA', 'Curso (Matricula Atual) (Matricula)', 'Modelo'], 
                        value_vars=competencias, var_name='Competência', value_name='Nota')
...
df_curso_pivot.plot(kind='bar', stacked=True, ax=ax2, position=0, colormap='Set2', width=bar_width, label='Cursos')
